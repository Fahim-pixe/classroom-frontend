  ...
  <Card className="relative overflow-hidden transition-shadow hover:shadow-md">
    <CardContent className="p-6">
      ...
      {trend ? (
        ...
        <div className="mt-5 h-5" /> // Spacer for alignment when no trend
      )}
...
const QuickAction = ({ title, description, icon: Icon, to }: { title: string; description: string; icon: typeof Users; to: string }) => {
  const Link = useLink();
  return (
    <Link to={to} className="group flex min-h-[112px] items-center gap-5 rounded-2xl border border-border bg-card px-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md">
      ...
      <section className="flex flex-col justify-between gap-6 pb-10 pt-2 md:flex-row md:items-start">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="h-12 rounded-xl border-border bg-card px-5 text-base shadow-sm" asChild>
            <Link to="/subjects"><FileUp className="mr-2 h-5 w-5" /> Import</Link>
          </Button>
          <Button className="h-12 rounded-xl px-5 text-base shadow-sm" asChild>
            <Link to="/classes/create"><Plus className="mr-2 h-5 w-5" /> Add</Link>
          </Button>
        </div>
      </section>
...
const StudentDashboard = ({ currentUser }: AdminDashboardProps) => {
  const Link = useLink();
  const { data: dashboardRes, isLoading, isError } = useCustom({ url: "stats/dashboard", method: "get", queryOptions: { retry: 1 } }) as any;
  const dashboard = dashboardRes?.data?.data ?? dashboardRes?.data ?? {};
  const metrics = dashboard.metrics ?? {};
  ...
  if (isLoading) return <div className="flex min-h-96 items-center justify-center"><p className="animate-pulse text-sm text-muted-foreground">Loading your dashboard...</p></div>;
  ...
  return (
    <div className="min-h-full bg-background px-1 pb-10 text-foreground sm:px-2">
      <section className="flex flex-col justify-between gap-6 pb-10 pt-2 md:flex-row md:items-start">
        <div>
          <h1 className="text-4xl font-semibold tracking-tight text-foreground sm:text-5xl">Welcome back, {firstName}</h1>
          <p className="mt-4 text-lg text-muted-foreground">Stay on top of your classes, assignments and academic progress.</p>
        </div>
        <Button className="h-12 rounded-xl px-5 text-base shadow-sm" asChild><Link to="/classes"><Plus className="mr-2 h-5 w-5" /> Join class</Link></Button>
      </section>

      {isError && <div className="mb-6 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-sm text-destructive">Some personal dashboard data could not be loaded.</div>}

      <section className="grid gap-5 py-2 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="My classes" value={toNumber(metrics.myClasses)} icon={Layers3} />
        <StatCard label="Attendance" value={metrics.attendance == null ? " " : `${metrics.attendance}%`} icon={Users} />
        <StatCard label="Assignments" value={toNumber(metrics.assignments)} icon={PenLine} />
        <StatCard label="Upcoming" value={toNumber(metrics.upcoming)} icon={GraduationCap} />
      </section>

      <section className="mt-8 grid gap-6 xl:grid-cols-[1.3fr_1fr]">
        <Card className="shadow-sm"><CardHeader><CardTitle>Today&apos;s classes</CardTitle></CardHeader>
          <CardContent>
            {schedule.length === 0 ? <p className="text-sm text-muted-foreground">No classes scheduled today.</p> : <div className="space-y-3">{schedule.map((item: any) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-3"><div><p className="font-medium">{item.subjectName || item.name}</p><p className="text-sm text-muted-foreground">{item.name}</p></div><span className="text-sm text-muted-foreground">{item.schedules?.[0]?.startTime || "Scheduled"}</span></div>)}</div>}
          </CardContent>
        </Card>
        <Card className="shadow-sm"><CardHeader><CardTitle>Upcoming assignments</CardTitle></CardHeader>
          <CardContent>
            {assignments.length === 0 ? <p className="text-sm text-muted-foreground">No upcoming assignments.</p> : <div className="space-y-3">{assignments.map((item: any) => <div key={item.id} className="border-b border-border pb-3 last:border-0"><p className="font-medium">{item.title}</p><p className="text-sm text-muted-foreground">{item.className} • {item.dueAt ? new Date(item.dueAt).toLocaleDateString() : "No due date"}</p></div>)}</div>}
          </CardContent>
        </Card>
      </section>
    </div>
